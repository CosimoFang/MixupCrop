...
def main(args):
    ...
    if args.dataset.name == "cifar10":
        ...
        feature = 100
        print(f"out_feature: {feature}")
    else:
        feature = 10
    classifier = nn.Linear(in_features=model.output_dim, out_features=feature, bias=True).to(args.device)
    ...
    for epoch in range(0, args.eval.num_epochs):
        loss_meter.reset()
        model.eval()
        classifier.train()
        ...
        for idx, (images, labels) in enumerate(train_loader):
            ...
            # print(f"lr: {lr}, loss: {loss_meter.val}, loss_avg: {loss_meter.avg}")
        print(f"Epoch: {epoch}, Acc: {acc_meter.val}, Acc_avg: {acc_meter.avg}")
    classifier.eval()
    correct, total = 0, 0
    acc_meter.reset()
